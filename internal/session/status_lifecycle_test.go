...
// TestStatusCycle_ShellSessionWithCommand verifies the full lifecycle for a
// non-interactive shell command. While the command is still inside tmux's
// startup window and has neither prompt output nor busy markers, the runtime
// contract is to remain StatusStarting rather than flip to idle.
func TestStatusCycle_ShellSessionWithCommand(t *testing.T) {
	...
	// A non-interactive shell command such as sleep does not expose a prompt, so
	// tmux keeps it in the startup state until either prompt/busy evidence appears
	// or the process exits.
	assert.Equal(t, StatusStarting, inst.Status, "shell session running sleep should remain starting during tmux startup window")
