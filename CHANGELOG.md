...
### Added
- GitHub Copilot CLI (`copilot`) as a first-class tool: selectable in session creation flows, resumable via `--resume`, with status detection, config support, and preflight binary checks.
- Copilot session details in the TUI, including detected session ID and detection timestamp.
- Copilot troubleshooting guidance in the documentation.
