...
1. Write it immediately to `.squad/decisions/inbox/copilot-directive-{timestamp}.md` using the format defined in `.squad/templates/decision-format.md`:
   ```
   ### {ISO-date}: {title}
...
prompt: |
  ...
  **Requested by:** {current user name}
  ...
  If you made a meaningful decision that would be of interest - in your estimation - to even one other person on the team, write to .squad/decisions/inbox/{name}-{brief-slug}.md using the format in .squad/templates/decision-format.md (fields: ### {date}: {title}, **By:**, **Status:**, **What:**, **Why:**).
...
* Agents write decisions to individual drop files: `.squad/decisions/inbox/{agent-name}-{brief-slug}.md` using the canonical format in `.squad/templates/decision-format.md`
* Each file contains exactly one decision block: `### {date}: {title}`, then `**By:**`, `**Status:**`, `**What:**`, `**Why:**` — in that order
...
prompt: |
  ...
  **Requested by:** {current user name}
  ...
  AFTER work:
  1. Using the current timestamp, captured using a tool for access to the system clock, write both to:
     - APPEND to .squad/agents/{name}/history.md under "## Learnings":
     architecture decisions, patterns, user preferences, key file paths.
     - If you made a team-relevant decision, write to
     .squad/decisions/inbox/{name}-{brief-slug}.md using the format in
     .squad/templates/decision-format.md:
     ### {date}: {title}
     **By:** {Name} ({Role})
     **Status:** {Decided|Approved|Observation|Action required — {x}|Completed}
     **What:** {1-3 sentences}
     **Why:** {1-3 sentences}
  2. SKILL EXTRACTION: If you found a reusable pattern, write/update
...
1. Run `gh --version`. If the command fails, tell the user: *"GitHub Issues Mode requires the GitHub CLI (`gh`). Install it from* [*https://cli.github.com/*](https://cli.github.com/) *and run `gh auth login`."*
