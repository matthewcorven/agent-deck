...
After making a decision others should know, write it to `.squad/decisions/inbox/{my-name}-{brief-slug}.md` using the format in `.squad/templates/decision-format.md` (fields: `### {date}: {title}`, `**By:**`, `**Status:**`, `**What:**`, `**Why:**`) — the Scribe will merge it.
